...
+ (instancetype)metaWithClass:(Class)cls {
    ...
    if (!meta || meta->_classInfo.needUpdate) {
        meta = [[_YYModelMeta alloc] initWithClass:cls];///得到类的信息YYClassInfo
        if (meta) {
            dispatch_semaphore_wait(lock, DISPATCH_TIME_FOREVER);
 ...
 */
///根据setter 的 imp设置值到变量中
#pragma mark - 从model中取出imp 根据信息将value设置到对一个的值
...
+ (instancetype)modelWithDictionary:(NSDictionary *)dictionary {
    ...
    Class cls = [self class];//Model类型
    _YYModelMeta *modelMeta = [_YYModelMeta metaWithClass:cls];//内部有缓存获取处理
