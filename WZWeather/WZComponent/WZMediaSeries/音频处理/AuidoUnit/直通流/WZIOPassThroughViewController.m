...
//    [session setPreferredSampleRate:rate error:&error];//自定义 会有上限，这个上限根据设备硬件而言
    ...
    //out to in
    AudioUnitConnection connection;//配置audioUnit的elemen
    connection.sourceAudioUnit = audioUnit; //连接源 根据componentSubType判定使用
    ...
    //之后： output scope   element1 -》application -》 input scope element0
    if (noErr != err) {
        [self showErrorStatus:err];
    }
}
...
//        UInt32 flag = 1;
//        OSStatus err = AudioUnitSetProperty(audioUnit,
//                                            kAudioOutputUnitProperty_EnableIO,//默认开启的 所以不用激活
